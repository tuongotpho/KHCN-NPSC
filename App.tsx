...
const App: React.FC = () => {
  ...
  const dashboardStats = useMemo(() => {
    ...
    return { total, currentYearCount, levelDist };
  }, [initiatives]);
  ...
  const generateLeadershipInsight = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Hãy phân tích tình hình sáng kiến của đơn vị dựa trên ${initiatives.length} bản ghi hiện có.`,
        config: { systemInstruction: AI_SYSTEM_INSTRUCTION }
      });
        ...
        {activeTab === 'list' ? (
          ...
          <div className="space-y-10 animate-slide">
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm text-center">
                  <div className="p-4 bg-amber-50 text-amber-600 rounded-2xl mb-4 mx-auto w-fit"><Activity size={24} /></div>
                  <p className="text-slate-400 font-bold uppercase text-[10px] mb-1">Năm {new Date().getFullYear()}</p>
                  <h4 className="text-4xl font-black text-slate-900">{dashboardStats.currentYearCount}</h4>
                </div>
             </div>
             <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                   <h3 className="text-xl font-black text-slate-900 uppercase">Phân tích chiến lược từ AI</h3>
                   <button onClick={generateLeadershipInsight} disabled={isGeneratingInsight} className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-black flex items-center gap-2 hover:bg-slate-800 transition-all">{isGeneratingInsight ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />} Phân tích dữ liệu mới</button>
                </div>
                <div className="min-h-[200px] flex items-center justify-center border-2 border-dashed border-slate-100 rounded-[2rem] p-8">
                   {aiInsight ? <div className="text-slate-700 leading-relaxed font-medium whitespace-pre-wrap">{aiInsight}</div> : <div className="text-slate-300 font-bold italic uppercase text-xs">Chưa có bản phân tích nào được tạo</div>}
                </div>
             </div>
          </div>
        ) : (
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-6xl max-h-[90vh] shadow-2xl overflow-hidden flex flex-col animate-slide">
                ...
                {batchResults.length === 0 ? (
                  ...
                  <div className="space-y-6">
                     <div className="flex items-center justify-between bg-emerald-50 p-6 rounded-3xl border border-emerald-100">
                        ...
                        <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest bg-white/50 px-3 py-1 rounded-full">Quản trị viên vui lòng xác nhận dữ liệu</p>
                     </div>
                           ...
                           <tbody className="divide-y divide-slate-100">
                              {batchResults.map((item) => (
                                <tr key={item.tempId} className={`transition-colors ${item.selected ? 'bg-emerald-50/20' : 'bg-slate-50/50 opacity-40'}`}>
                                   ...
                                   <td className="px-6 py-4 text-slate-600 font-medium"><span className="line-clamp-1">{item.authors?.join(', ')}</span></td>
                                   ...
                                   <td className="px-6 py-4"><div className="flex items-center justify-center gap-2"><button onClick={() => setEditingBatchItem(item)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-all"><Edit size={16} /></button><button onClick={() => setBatchResults(prev => prev.filter(r => r.tempId !== item.tempId))} className="p-2 text-rose-400 hover:bg-rose-50 rounded-lg transition-all"><Trash2 size={16} /></button></div></td>
                                </tr>
                              ))}
                           </tbody>
                        </table>
             ...
             {batchResults.length > 0 && (
                <div className="p-8 border-t border-slate-100 bg-slate-50 flex gap-4">
                   ...
                   <button onClick={saveBatchResults} className="flex-[2] py-4 bg-emerald-600 text-white rounded-2xl font-black shadow-xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 uppercase text-xs tracking-widest">Xác nhận lưu {batchResults.filter(r => r.selected).length} bản ghi <CheckCircle2 size={20} /></button>
                </div>
             )}
          </div>
        </div>
      ...
      {editingBatchItem && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in zoom-in duration-200">
           <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl flex flex-col overflow-hidden">
              <div className="p-8 border-b border-slate-100 flex items-center justify-between"><h3 className="text-xl font-black text-slate-900 uppercase">Hiệu chỉnh dữ liệu trích xuất</h3><button onClick={() => setEditingBatchItem(null)} className="p-2 hover:bg-slate-100 rounded-lg"><X size={20}/></button></div>
              <div className="p-8 space-y-4">
                 ...
                 <div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 ml-2">Tác giả (Cách nhau bằng dấu phẩy)</label><input className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" value={editingBatchItem.authors?.join(', ')} onChange={(e) => setEditingBatchItem({...editingBatchItem, authors: e.target.value.split(',').map(s => s.trim())})} /></div>
              </div>
