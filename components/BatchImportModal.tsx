...
import { X, UploadCloud, FileText, Check, AlertTriangle, Loader2, Save, Wand2, ShieldAlert, Sparkles, CheckCircle2, Info } from 'lucide-react';
import { extractInitiativesFromPDF, checkSimilarityBatch } from '../services/aiService';
import { db } from '../services/firebase';
import { BatchItem, InitiativeLevel, Initiative } from '../types';
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          ...
          // Bước 2: Lấy dữ liệu cũ để so sánh
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 dark:border-slate-800">
        ...
        {step === 'review' && (
          <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex gap-4">
            <button onClick={() => setStep('upload')} className="flex-1 py-4 border-2 border-slate-200 rounded-2xl font-black text-slate-400 uppercase text-xs">Hủy</button>
            <button 
              onClick={async () => {
                setIsSaving(true);
                const batch = db.batch();
                extractedItems.filter(i => i.selected).forEach(item => {
                  const docRef = db.collection('initiatives').doc();
                  batch.set(docRef, { ...item, phase: 'Hoàn thành', result: 'Đạt', reward: '', similarityInfo: item.similarity });
                });
                await batch.commit();
                onClose();
              }}
              className={`flex-[2] py-4 ${activeTheme.primary} text-white rounded-2xl font-black shadow-lg uppercase text-xs flex items-center justify-center gap-2`}
              disabled={isSaving}
            >
              {isSaving ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
              {isSaving ? 'Đang lưu...' : `Lưu ${extractedItems.filter(i => i.selected).length} sáng kiến`}
            </button>
          </div>
        )}
      </div>
